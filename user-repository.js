...
// BaseRepository is defined in task-repository.js and should be available
// If not available yet due to script loading order, define it here
...
class UserRepository extends BaseRepository {
    ...
    async update(id, updates) {
        try {
            ...
            // Apply updates using User model methods
            if (updates.fullName !== undefined || updates.email !== undefined) {
                currentUser.updateProfile(updates.fullName, updates.email);
            }

            if (updates.preferences !== undefined) {
                currentUser.updatePreferences(updates.preferences);
            }
            ...
            // For other properties, try direct assignment if they exist
            Object.keys(updates).forEach(key => {
                if (!['fullName', 'email', 'preferences', 'isActive'].includes(key)) {
    ...
    async findVerifiedUsers() {
        // Since User model doesn't have isVerified, return all active users
        return this.findAll({ isActive: true });
    }
    ...
    async getStatistics() {
        ...
        stats.recentlyActive = users.filter(user => {
            return user.lastLoginAt && new Date(user.lastLoginAt) > oneDayAgo;
        }).length;
    ...
    async getUsersByActivity(days = 30) {
        ...
        return {
            active: users.filter(user => {
                return user.lastLoginAt && new Date(user.lastLoginAt) > cutoffDate;
            }),
